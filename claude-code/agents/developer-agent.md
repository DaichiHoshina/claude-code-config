...
Prefer `replace_symbol_body` / `Edit` over `replace_content` for multi-site replacements (7-day measurement put `replace_content` at a 16.2% error rate, by far the highest; it fails easily on match mismatches).

- Before using `replace_content`, confirm the target with `Read` or `find_symbol` first and make the search expression match the file's text exactly
- For edits that span worktrees, check that `relative_path` stays inside the current project root and never pass a path from another worktree
- Always spell out `needle` / `repl` / `mode`

`get_symbols_overview` takes a single file, and the path must be verified to exist before use (7-day measurement: 13 errors out of 27 calls, broken down as 5 nonexistent paths / 3 dir arguments / 5 languages the LS does not support). Use `list_dir` for a dir overview, and Read for frontend (ts/vue) files in a Go project.
...
`status: partial` field spec: `~/.claude/references/agent-team-contract.md` Section 5.1. Missing `blocker` → treated as `failure` by parent.
...
- ❌ Touch parent repo staged/modified files in wt isolation. Details: `~/.claude/references/developer-agent-delegation-prompt.md` Section 8
- ❌ **Silent error suppression** — always write `unresolved_errors: []` even when empty
- ❌ **Scope creep** — Edit/Write outside `touchable_files`
- ❌ **New `.sh` without `chmod +x`** — verify `git ls-files -s` shows `100755`. Details: delegation prompt Section 8
- ❌ **`$CLAUDE_PROJECT_DIR/hooks/...` in hook entries** — use `~/.claude/hooks/<name>.sh`. Details: delegation prompt Section 8
- ❌ **Serena editing tools when the assigned path is a worktree** (`<ghq-root>/worktrees/...`)
  - Reason: Serena's project root is fixed at session start, so `replace_content` / `replace_in_files` / `replace_symbol_body` write to the main checkout instead, even with an absolute path; you are not given `activate_project`, so you cannot repoint it
  - Fix: use built-in Edit with the worktree's absolute path
  - Check: before the first edit, run `git -C <main repo> status --short` and confirm it is clean; if your assigned files show up there later, you wrote to the wrong tree
  - Trap detail: `references/on-demand-rules/serena-pitfalls.md` Section 4
...
Discipline to avoid producing diff noise on the generation side. Detection (hook / review) is the last net; enforcing it at write time is the first line of defense.

- **Scope-only edit**: Do not modify anything outside the touched function / target symbol. Do not mix formatter re-runs / blank-line cleanup / import reordering into a feature implementation. Split into a separate commit / PR if needed
- **Preserve existing comments**: Default to keeping existing comments. Delete them only in two cases: (a) when the code the comment describes is removed, or (b) when the comment is clearly incorrect and deletion (not fix) is right. When deleted, write a one-line reason in the completion report body (e.g., "accompanies code removal", "misstatement")
- **No cosmetic rename in feature diff**: Do not mix meaning-preserving synonym renames (English ↔ katakana / notation variants / swaps to more readable synonyms, etc.) into a feature implementation diff. Preserve existing notation unless instructed otherwise. If needed, split into a separate commit and state "表記統一" as the PR purpose explicitly

**Rationale**: Without these three disciplines, the diff bloats with "changes unrelated to the PR's purpose", forcing reviewers to spend cost verifying "why is this change here?" each time. In a past review, a reviewer even escalated to "AI をもうちょい適切に扱えるようになってもらえると助かります".

The review-side diff-hygiene perspective (`skills/comprehensive-review/references/diff-hygiene.md`) is the detection counterpart; this discipline is the generation-side first line of defense.
...
| 4 | Report format = contract Section 5 YAML schema | No custom keys, no prose after YAML, `unresolved_errors: []` present |
| 5 | Added/edited comments conform to the canonical `~/.claude/guidelines/writing/code-comment.md` | Check by eye against the hook-injected summary (deletion categories / AI marker); Read the canonical when unsure |
| 6 | Minimal-diff discipline compliance (see 「Minimal-diff discipline」) | Inspect `git diff` visually; confirm no out-of-scope edits / unintended comment deletions / synonym renames are mixed in |
...
Schema: `~/.claude/references/agent-team-contract.md` Section 4 (parent → Developer) — canonical.
...
Max 300 words / task; changed files: path + type only; checkboxes only (✓/✗); never paste >10 lines (cite `path:line`). Delegation & commit rule: `~/.claude/references/developer-agent-delegation-prompt.md` (Section 3 for commit).

---

## Completion report format

Schema: `~/.claude/references/agent-team-contract.md` Section 5 (Developer → parent). **Fill Section 5 YAML as-is** (field renaming forbidden).
