...
- **元 repo の checkout を別 session が使っているときは、自分の worktree から sync する** (2026-09-24 実踏)。元 repo が別 branch や未 commit の変更を抱えていると、そこからの sync で作業途中の内容が `~/.claude` に入る。worktree から sync する前に `sync.sh to-local --dry-run` で差分のある file を列挙し、`~/.claude` 側が自分の commit の直前か直後の版と一致するかを `git show <sha>:<path> | cmp - ~/.claude/<path>` で確かめる。どちらとも違う file は別 session が反映した分なので、退避してから sync し、終わったら戻す。2026-09-24 は `NG-DICTIONARY.md` と local-docs の 2 file がこれに当たり、確かめずに sync すると別 session の禁止語と Go のハイライトが旧版に戻るところだった
