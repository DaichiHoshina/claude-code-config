...
CLAUDE.md の `## Natural Language Triggers (常時掲載)` に掲載した行 (★) は session に常時載る。この doc だけに書かれた trigger は読み込まれず発火しにくい (2026-09-21 実測: 掲載のうち push とレビューの 2 本は自然文だけの発話が 290 件と 38 件あり発火していた。他の 3 本は自然文の発話自体が 0-1 件で発火するかは未確認。非掲載の 5 本は依頼が来ていながら起動 0 回だった)。発火させたい trigger は CLAUDE.md 側の表へ行を追加する。
...
| "観点全部でレビュー", "ガイドライン全載せでレビュー", "DDD 観点で", "CA 観点で" | `/review --full` (guideline 全載せ + 領域 memory 引き当て) |
...
| "Phase n の設計を説明して", "実装前に説明して" | `/explain <作業計画書 path> --phase <n>` (実装前 mode。層と責務 / 設計判断 / 処理の順序を説明し、Next で `/sdd-implement` へ) |
...
| "判定だけ", "mode 判定のみ", "判定して実装しない" | `/plan --mode-only <task>` (実装せず実行 mode の判定だけ返す) |
...
| "実行方法選んで" | `/plan --go <task>` (mode 判定 + 実装。判定だけなら `--mode-only`) |
...
| ★ "リファクタ", "リファクタリング", "構造改善" | `/refactor <target>` (言語別 guideline を auto-load して構造改善) |
| ★ "エラーが出ている", "原因を調べて", "debug して" | `/diagnose <症状>` (log 分析 → 真因特定 → 修正案。RCA は `root-cause` skill) |
| ★ "verify して", "動作確認" | `/verify-once` (実装変更を実際に動かして smoke 検証。lint と test の bundle は `/lint-test`) |
| "review して修正して push" | `/review --push` (`/review` → 全 fix → 再 review → `/git-push --pr`) |
...
| repo 側 `/spec:design` / `/spec:plan` / `/spec:execute` (repo 側の正この rule dir) で設計書 / 計画書 / 実装を求められた | `/sdd-design` / `/sdd-plan` / `/sdd-implement` へ誘導する (repo 側の出力先 `.claude/docs/` は write 禁止で成果物を保持できない。08-28 〜 09-03 の 32 回発火で file 0 件を実測)。`/spec:review` `/spec:quality` は相談用途としてそのまま使う |
| ★ "詳細設計して", "Phase n の実装方法を決めて", "実装前に調べて" | `/sdd-phase-design <作業計画書 path> --phase <n>` (Phase 1 つの実装形を既存 code の調査から決める。省略判定に当たる Phase は file を作らない) |
| ★ "Phase n 実装して", "計画書の次の Phase やって", "作業計画書通りに実装" | `/sdd-implement <作業計画書 path> --phase <n>` (Phase 1 つを実装し、完了報告で `/sdd-review` を Next に記載する) |
| ★ "Phase をレビューして", "作業計画書と突き合わせてレビュー" | `/sdd-review <作業計画書 path> --phase <n>` (`/review` を Phase の scope で当てる。修正するもの 0 件なら `/explain` を Next に記載する) |
| ★ "作業計画書作って", "作業計画書に分けて", "PR 構成決めて" | `/sdd-plan <Design Doc path>` (Phase = PR の作業計画書。repo template があれば節構成を合わせる) |
| ★ "design doc 書いて", "DD 起こして", "設計書書いて" | `/sdd-design <feature>` (12-section 設計 md を team 共有用に起草) |
| ★ "全体像把握して", "issue 読んで整理して", "タスク理解して" | `/prepare <issue URL / PRD path>` (出典を集めて要求 `R-n` / 現状 / 未確定点を整理、read-only。出口は `/prd` か `/sdd-design`) |
| ★ "PRD 書いて", "要件整理して" | `/prd <feature>` (PRD 起草) |

## 規模による入口 (3 track)

上の表は command 名を含む発話を command へ対応づけるもので、規模の判定を経ない。command 名を含まない依頼 (「このバグを修正して」「〜機能を追加して」) では、変更の規模で入口が変わる。
...
判定と track ごとの成果物は `design-phase-flow.md` 「Route selection (3 track)」が canonical となる。上の表のうち `/sdd-design` / `/sdd-plan` / `/sdd-phase-design` / `/sdd-implement` / `/sdd-review` の行は、大きい開発の track にだけ当たる。
