...
5. **複数 file にまたがる用語・命名の一括置換** — 置換対象が 2 file 以上か 20 件以上なら、編集前に変更一覧 (置換の対応表 / file ごとの件数 / 変えない語とその理由) を出して合意を取る。置換は flag 名・判定 script・trigger 表まで波及し、途中で決める点が多い (2026-09-24 に一覧なしで 6 file を並列に書き換え、全変更を元に戻した)。1 file の数か所なら確認なしで進める
