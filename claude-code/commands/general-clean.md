...
3. launchd / cron も同様に扱う。plist が `launchctl list` に無いのは「そのマシンで load していない」以上の意味を保持しない。メイン PC で `launchctl list | grep com.claude` を取り、載っている job は削除候補に含めない (同日に 6 本を「どこにも load されていない」として消し、メイン PC では全部動いていたので revert した)

| 対象 | 計測 | 誤りやすい点 (2026-08-29 実踏) |
|---|---|---|
| command 利用 | transcript (`~/.claude/projects/**/*.jsonl`、mtime -90) の `<command-name>/x</command-name>` を数える | `commands/x.md` の file 名 grep は過大になりやすい。<br>- 原因: ai-tools 保守 session の Read / Edit も含めてしまう<br>- 対策: file 名で数えるなら ai-tools の project dir を対象外にする |
...
- 参照 grep は常に `rg --no-ignore --hidden` で行う。repo root の `.ignore` が `*.md` を広く対象外にするため、付けないと `CLAUDE.global.md` / `references/` / `agents/README.md` が hit せず参照数を過小に出す (2026-09-05 に「参照 4 か所」と書いて実測は 19 file だった)
...
| 状態 | 扱い |
|---|---|
| 作成 30 日超 かつ 発火 0 かつ 自然文の依頼 0 件 かつ 参照 0 | 削除候補 |
| 発火 0 だが自然文の依頼が 1 件以上 | **削除しない**。導線の不足として扱い、trigger を CLAUDE.md の表へ上げる (下記 Step 2b) |
| 作成 30 日未満 | 次回へ繰り越す (判定の母数が足りない) |
| 発火 0 だが他 repo で読まれた | 保留 (次回再計測) |
...
## Step 2b: 発火 0 を「不要」と読む前に (必須)

発火 0 の原因は 2 つある。需要が無いのか、呼ぶ言い回しが session に載っていないのかを先に分ける。分けずに消すと、依頼が来ているコマンドを消す。

1. 自然文の依頼を数える。母集団は `~/.claude/history.jsonl` (user の prompt 履歴) で、transcript より雑音が少ない

```bash
python3 -c "
import json,re
rows=[json.loads(l) for l in open('$HOME/.claude/history.jsonl')]
pat=r'<対象の言い回しを | で並べる>'
hit=[r for r in rows if re.search(pat,r['display'])]
print(len(hit)); [print(' ',r['display'][:70]) for r in hit[-5:]]"
```

2. hit した文面を目で見る。件数だけで判定しない (`/handoff` を「別セッション」「引き継」で数えると 2 件 hit するが、中身は worktree の相談と `HANDOFF.md` という file の話で、command への依頼ではなかった。2026-09-21 実踏)
3. 依頼が 1 件以上あるなら、その command は需要がある。`CLAUDE.global.md` の `## Natural Language Triggers` の表に行があるかを見る
4. 表に無いなら、それが発火しない原因だ。`references/natural-language-triggers.md` は on-demand なので session に載らず、書いても読まれない。行を CLAUDE.md 側の表へ追加して次回まで再計測する (2026-09-21 実測: CLAUDE.md 掲載のうち push とレビューの 2 本は自然文だけで発火していた。他の 3 本は自然文の発話が 0-1 件で未確認。非掲載の 5 本は依頼が来ていながら起動 0 回だった)
5. 依頼も 0 件なら削除候補に進む

**次回の再判定 (2026-10-21)**: `/norm-apply` (2026-08-30 作成) / `/review-text` (09-16) / `/review-queue` (09-06) / `/general-clean` (09-05) / `/ld` (09-03) / `/handoff` (08-29) は 2026-09-21 時点で作成 30 日未満のため繰り越した。
