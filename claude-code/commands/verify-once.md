...
2. **unit**: `bats -r -j 8 tests/` matched test run (GNU parallel なし環境は `-j 8` を外す) (filter by changed file)。exit code は `bats ... > /tmp/x.out 2>&1; echo $?` の形で直接見る — `bats | tail` 等の pipe は exit code を隠し、fail を全 pass と誤読する (d73e4e2 regression の見逃し実例)
