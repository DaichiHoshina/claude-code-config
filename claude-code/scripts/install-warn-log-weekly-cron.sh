...
launchd_check_worktree_repo_root WARN_LOG_CRON_ALLOW_WT "$REPO_ROOT" "ai-tools-wt-" \
  "./scripts/install-warn-log-weekly-cron.sh --repo \$HOME/ghq/github.com/<owner>/ai-tools/claude-code" || exit $?

launchd_check_executable "${REPO_ROOT}/scripts/warn-log-weekly.sh" "${REPO_ROOT}/scripts/warn-log-weekly.sh" || exit $?

CALENDAR_XML="$(launchd_calendar_interval_weekly 10 0)"
PLIST_CONTENT="$(launchd_render_plist "$LABEL" \
  "$CRON_LOG" "$CRON_LOG" "$CALENDAR_XML" \
  "/bin/bash" "-lc" "cd ${REPO_ROOT} && bash ./scripts/warn-log-weekly.sh")"
